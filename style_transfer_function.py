...
def exact_feature_distribution_matching(content_feat, style_feat):
    ...
    value_content, index_content = torch.sort(content_feat.contiguous().view(B,C,-1))  # sort conduct a deep copy here.
    value_style, _ = torch.sort(style_feat.contiguous().view(B,C,-1))  # sort conduct a deep copy here.
    inverse_index = index_content.argsort(-1)
    new_content = content_feat.contiguous().view(B,C,-1) + (value_style.gather(-1, inverse_index) - content_feat.contiguous().view(B,C,-1).detach())

    return new_content.contiguous().view(B, C, W, H)
