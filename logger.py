...
class Logger(object):
    def __init__(self, log_dir=settings.log_dir, img_dir=settings.imgs_dir,
                 monitoring='tensorboard', monitoring_dir = path.join(settings.log_dir, 'monitoring')):
        self.stats = dict()
    ...
    def add_imgs(self, imgs, class_name, it):
        ...
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        outfile = os.path.join(outdir, '%08d.jpg' % it)
        imgs = imgs / 2 + 0.5
        imgs = torchvision.utils.make_grid(imgs)
        if it % 1000 == 0:
            torchvision.utils.save_image(imgs, outfile, nrow=1)
        if it % 1000 == 0:
